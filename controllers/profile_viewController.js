...
const profile_viewController = {
				...
				Posts.findOne({email: email}, function(err, results) {
					if(results != null) {
						db.findMany(Posts, query,function(err, resultz) {
							...
							User.findOne({email: emailUser}, function(err, resultf) {
								if (resultf.online == true) {
	...
	getRate: function (req, res) {
		User.findOne({email: req.params.emailUser}, function(err, result) {
			if (result.online == true) {
				res.render('rate', {email: req.params.email, emailUser: req.params.emailUser});
			}
